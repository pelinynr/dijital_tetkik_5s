...
if (Test-Path ".\mvnw.cmd") {
  & ".\mvnw.cmd" "-Dmaven.repo.local=$repository" spring-boot:run
  exit $LASTEXITCODE
}

$maven = Get-Command mvn -ErrorAction SilentlyContinue
if ($maven) {
  & $maven.Source "-Dmaven.repo.local=$repository" spring-boot:run
  exit $LASTEXITCODE
}
...
if (-not $cached) {
  throw "Maven bulunamadı. https://maven.apache.org/download.cgi adresinden Maven kurup terminali yeniden açın."
}

& $cached.FullName "-Dmaven.repo.local=$repository" spring-boot:run
exit $LASTEXITCODE
