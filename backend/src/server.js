...
app.get("/api/admin/area-config",auth,async(request,response,next)=>{if(request.user.role!=="admin")return response.status(403).json({error:"Yalnızca sistem yöneticisi erişebilir"});try{const areas=await pool.query(`SELECT a.id,a.area_code,a.name,a.qr_token,u.id AS owner_id,u.full_name AS owner_name,u.email AS owner_email FROM areas a LEFT JOIN users u ON u.id=a.owner_id WHERE a.active=true ORDER BY a.area_code`);const users=await pool.query("SELECT id,full_name,email FROM users WHERE role='area_owner' AND active=true ORDER BY full_name");response.json({areas:areas.rows,users:users.rows})}catch(e){next(e)}});
